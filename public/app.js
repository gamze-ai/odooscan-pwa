...
function renderContactStep() {
  ...
  let html = `
    ...
    <div class="match-header">
      <div style="font-size:11px;color:var(--muted);font-weight:700;letter-spacing:1px;margin-bottom:4px">KİŞİ</div>
  ...
  // DÜZELTME 2: "Yeni Kontak Oluştur" butonu — basılınca manuel alanlar modal'ı açar
  html += `
    <button class="btn-primary" onclick="showNewContactModal()" style="background:var(--accent2);color:#fff">
      ➕ Yeni Kontak Oluştur
      <span style="display:block;font-size:11px;font-weight:400;margin-top:2px;opacity:.8">${esc(name)} adıyla kayıt açılacak</span>
    </button>`;
...
function renderManualFields() {
  ...
  html += makeSelect('property_supplier_payment_term_id', 'Tedarikçi Ödeme Koşulu', opts.payment_terms || [], false);
  html += makeSelect('sale_currency_rate_type_id', 'Satış Kur Türü', opts.currency_rate_types || [], false);
...
function submitNewContact() {
  ...
  const required = ['user_id', 'property_payment_term_id'];
  ...
  // Opsiyonel alanlar
  ['property_supplier_payment_term_id', 'sale_currency_rate_type_id'].forEach(k => {
    const el = document.getElementById('mf-' + k);
    if (el) state.manualFields[k] = el.value ? parseInt(el.value) : null;
  });

  document.getElementById('new-contact-overlay').remove();
  state.selectedContactId = null; // Yeni kontak
  proceedToSave(null);
}

// DÜZELTME 1: Şirket adımı kaldırıldı — kontak seçilince direkt kaydet
function selectContact(id) {
  state.selectedContactId = id;
  // Mevcut kontak seçildi → direkt kaydet (şirket adımı yok)
  proceedToSave(null);
}
