...
function buildMatchQueue() {
  ...
  // Kartvizit kişisi — görüşülen alanlarda zaten varsa queue'ya ekleme
  const cardName = (cf.name || '').trim().toLowerCase();
  const normalize = s => s.trim().toLowerCase();
  const alreadyInQueue = queue.some(q => normalize(q.name) === cardName);
  // Ayrıca form görüşülen alanlarında da kontrol et
  const inFormFields = [ff.gorusulen_1, ff.gorusulen_2].some(
    n => n && normalize(n) === cardName
  );
  if (cardName && !alreadyInQueue && !inFormFields) queue.push({
    slot: 'card_contact', label: 'Kartvizit Kişisi',
    name: cf.name, phone: cf.phone || cf.mobile || '', email: cf.email || '',
...
function renderPersonStep(person, candidates, current, total) {
  ...
  // Kartvizit kişisi için yeni kontak butonu modal açar, diğerleri direkt oluşturur
  if (person.isCardContact) {
    html += `
    ...
    html += `
      <button class="btn-primary" onclick="selectPersonContact(null)" style="background:var(--accent2);color:#fff">
        ➕ Yeni Kontak Oluştur
        <span style="display:block;font-size:11px;font-weight:400;margin-top:2px;opacity:.8">${esc(person.name)} adıyla kayıt açılacak</span>
      </button>`;
  }
...
// DÜZELTME 2 + 3: Yeni kontak modal — dropdown seçeneklerini Odoo'dan çek
async function showNewContactModal() {
  ...
  overlay.innerHTML = `
    <div style="background:var(--surface);border-radius:20px 20px 0 0;padding:24px;width:100%;max-height:85vh;overflow-y:auto">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px">
        <h3 style="font-family:var(--font-head);font-size:17px">Yeni Kontak Bilgileri</h3>
      ...
      <div id="manual-fields-container">
        <div style="text-align:center;padding:20px;color:var(--muted)">
          ...
          Odoo'dan seçenekler yükleniyor...
        </div>
      </div>
      <button id="create-contact-btn" class="btn-primary" style="margin-top:16px" onclick="submitNewContact()">
        ✅ Kontağı Oluştur ve Kaydet
      </button>
    </div>`;

  document.body.appendChild(overlay);

  // Odoo'dan dropdown seçeneklerini çek
  try {
    ...
    const data = await res.json();
    state.odooOptions = data;
    renderManualFields();
  } catch (err) {
    document.getElementById('manual-fields-container').innerHTML = `
      <p style="color:var(--danger);font-size:13px">Odoo seçenekleri yüklenemedi: ${esc(err.message)}</p>
      <p style="color:var(--muted);font-size:12px;margin-top:8px">Manuel devam etmek için aşağıdan kaydedin.</p>`;
  }
}
...
function renderManualFields() {
  ...
  const makeSelect = (key, label, options, required = false) => {
    ...
    return `
      <div class="field-row">
        ...
        <select class="field-input" id="mf-${key}" onchange="state.manualFields['${key}']=this.value?parseInt(this.value):null"
  ...
  let html = `<p style="font-size:12px;color:var(--muted);margin-bottom:16px">Yıldızlı alanlar zorunludur.</p>`;
  html += makeSelect('user_id', 'Satış Temsilcisi', opts.users || [], true);
  html += makeSelect('property_payment_term_id', 'Müşteri Ödeme Koşulu', opts.payment_terms || [], true);
  html += makeSelect('property_supplier_payment_term_id', 'Tedarikçi Ödeme Koşulu', opts.payment_terms || [], true);
  html += makeSelect('sale_currency_rate_type_id', 'Satış Kur Türü', opts.currency_rate_types || [], true);

  document.getElementById('manual-fields-container').innerHTML = html;
}

function submitNewContact() {
  // Zorunlu alan kontrolü
  const required = ['user_id', 'property_payment_term_id', 'property_supplier_payment_term_id', 'sale_currency_rate_type_id'];
  for (const k of required) {
    const el = document.getElementById('mf-' + k);
    if (el) state.manualFields[k] = el.value ? parseInt(el.value) : null;
    if (!state.manualFields[k]) {
  ...
  // Kartvizit kişisi için yeni kontak — manualFields kaydedildi, kuyruğa devam
  state.matchResults.push({
    slot: state.matchQueue[state.currentMatchIdx]?.slot || 'card_contact',
    contactId: null, isNew: true, isCardContact: true,
...
async function proceedToSave(companyId) {
  ...
  // Kartvizit kontağı için manualFields (yeni oluşturulacaksa)
...
function saveToHistory(result) {
  const hist = JSON.parse(localStorage.getItem('scan_history') || '[]');
  hist.unshift({ formFields: state.formFields, cardFields: state.cardFields, result, scanMode: state.scanMode, timestamp: new Date().toISOString() });
