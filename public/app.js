...
// Backend URL sabit — admin dışındaki kullanıcılar ayarlardan değiştiremez
const DEFAULT_BACKEND_URL = 'https://odooscan-backend.onrender.com';
...
async function doLogin() {
  ...
  // Backend URL: ayarlarda varsa onu kullan, yoksa default
  const cfg = getConfig();
  const backendUrl = cfg.backendUrl || DEFAULT_BACKEND_URL;

  try {
    const res = await fetch(backendUrl.replace(/\/$/, '') + '/api/login', {
    ...
    if (data.success) {
      ...
      // Login'den gelen config bilgilerini kaydet — showScreen'den ÖNCE
      const c = getConfig();
      c.backendUrl = backendUrl;
      c.url = data.odoo_url || c.url || '';
      c.db = data.odoo_db || c.db || '';
      c.username = email;
      c.apiKey = data.api_key;
      saveConfig(c);
      // State'i de güncelle ki refreshHome doğru çalışsın
      state.odooUrl = c.url;
      state.odooDB = c.db;
      showScreen('home');
      refreshHome();
...
async function processBulkFile() {
  ...
  // AI'a gönder
  try {
    const res = await fetch((cfg.backendUrl || DEFAULT_BACKEND_URL).replace(/\/$/, '') + '/api/extract', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
...
async function processNextInQueue() {
  ...
  try {
    if (person.isCompany) {
      // Şirket araması
      const res = await fetch((cfg.backendUrl || DEFAULT_BACKEND_URL).replace(/\/$/, '') + '/api/check-contact', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
...
async function showNewContactModal() {
  ...
  if (!state.odooOptions?.users?.length) {
    try {
      const res = await fetch((cfg.backendUrl || DEFAULT_BACKEND_URL).replace(/\/$/, '') + '/api/odoo-options', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
...
async function loadStatesForCompany(countryId) {
  ...
  const cfg = getConfig();
  try {
    const res = await fetch((cfg.backendUrl || DEFAULT_BACKEND_URL).replace(/\/$/, '') + '/api/states-by-country', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
...
async function proceedToSave(companyId) {
  ...
  try {
    const res = await fetch((cfg.backendUrl || DEFAULT_BACKEND_URL).replace(/\/$/, '') + '/api/send-to-odoo', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        odoo_config: { url: cfg.url || '', db: cfg.db || '', username: cfg.username || '', api_key: cfg.apiKey || '' },
...
async function saveToHistory(result) {
  ...
  const item = {
    ...
    scannedBy: user?.name || null,
  };

  // Backend'e kaydet
  try {
    await fetch(backendUrl.replace(/\/$/, '') + '/api/history/save', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        user_email: item.user_email,
        form_fields: item.formFields,
        card_fields: item.cardFields,
        extra_cards: item.extraCards,
        result: item.result,
        scan_mode: item.scanMode,
        file_mime: item.fileMime,
        file_name: item.fileName,
        bulk_session_id: item.bulkSessionId,
  ...
  try {
    const hist = JSON.parse(localStorage.getItem('scan_history') || '[]');
    hist.unshift({...item, fileBase64: null});
...
async function renderHistory(searchQuery, filterUser, filterCompany, filterFuar, filterAksiyon) {
  const body = document.getElementById('history-body');
  const cfg = getConfig();
  const backendUrl = cfg.backendUrl || DEFAULT_BACKEND_URL;
  const user = state.currentUser || JSON.parse(localStorage.getItem('odoo_user') || 'null');

  // Backend'den çek
  let hist = [];
  try {
    body.innerHTML = '<div class="ai-loader show" style="padding:40px 0"><div class="pulse-ring"></div><p>Geçmiş yükleniyor...</p></div>';
    const res = await fetch(backendUrl.replace(/\/$/, '') + '/api/history/' + encodeURIComponent(user?.email || ''));
    const data = await res.json();
    if (data.success) {
      hist = data.items || [];
      // Backend çalışıyorsa localStorage'ı güncelle
      try {
        localStorage.setItem('scan_history', JSON.stringify(hist.slice(0, 50)));
      } catch(e) {}
    } else {
      // Backend'den hata gelirse localStorage'a bak
  ...
  // hist-count-label güncelle
  const histLabel = document.getElementById('hist-count-label');
  if (histLabel) histLabel.textContent = hist.length + ' tarama';
...
function showHistoryDetail(idx) {
  ...
  const makeEditable = (label, fieldset, key, value) => {
    ...
    return `<div style="display:flex;gap:8px;padding:8px 0;border-bottom:1px solid var(--surface2);align-items:flex-start;min-width:0">
      <div style="font-size:11px;color:var(--muted);width:100px;min-width:100px;flex-shrink:0;padding-top:2px">${safeLabel}</div>
      <input style="font-size:13px;color:var(--text);background:transparent;border:none;flex:1;outline:none;border-bottom:1px solid var(--accent2);min-width:0;width:0;word-break:break-word"
        value="${esc(value || '')}"
...
async function updateOdooFromHistory(idx) {
  ...
  const cfg = getConfig();
  try {
    const res = await fetch((cfg.backendUrl || DEFAULT_BACKEND_URL).replace(/\/$/, '') + '/api/update-visit', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        odoo_config: { url: cfg.url || '', db: cfg.db || '', username: cfg.username || '', api_key: cfg.apiKey || '' },
...
async function clearHistory() {
  ...
  const user = state.currentUser || JSON.parse(localStorage.getItem('odoo_user') || 'null');
  try {
    await fetch(backendUrl.replace(/\/$/, '') + '/api/history/' + encodeURIComponent(user?.email || ''), {
      method: 'DELETE'
    });
  } catch(e) {}
...
async function testConnection() {
  ...
  const user = JSON.parse(localStorage.getItem('odoo_user') || 'null');
  const backendUrl = cfg.backendUrl || DEFAULT_BACKEND_URL;
  // URL ve DB: ayarlarda varsa onu kullan, yoksa login'den gelen değeri kullan
  const url = cfg.url || '';
  const db = cfg.db || '';
  const username = cfg.username || user?.email || '';
  const api_key = cfg.apiKey || user?.api_key || '';
  ...
  try {
    const res = await fetch(backendUrl.replace(/\/$/, '') + '/api/test-connection', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ url, db, username, api_key }),
    });
    const data = await res.json();
