...
function renderHistory() {
  ...
  // Toplu yükleme oturumlarını grupla
  const seenSessions = {};
  let html = '';
  ...
  hist.forEach((item, idx) => {
    ...
    // Toplu yükleme grup başlığı
    if (sid && !seenSessions[sid]) {
      const groupItems = hist.filter(h => h.bulkSessionId === sid);
      ...
      html += '<div style="display:flex;align-items:center;gap:8px;margin:16px 0 8px">' +
        ...
        '<div style="font-size:11px;color:var(--muted)">' + groupItems.length + ' kayıt · ' +
