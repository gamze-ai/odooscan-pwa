...
function renderNewContactForm(prefill) {
  ...
  // Ülke dropdown — seçilince şehirler dinamik yüklenir
  const countries = opts.countries || [];
  const countrySel = countries.map(c =>
    `<option value="${c.id}" ${prefill.country_id == c.id ? 'selected' : ''}>${esc(c.name)}</option>`
  ).join('');
  html += `
    <div class="field-row">
      <div class="field-label">Ülke</div>
      <select class="field-input" id="nc-country_id" onchange="loadStatesForCountry(this.value)"
        style="background:var(--surface2);color:var(--text);border:1px solid rgba(255,255,255,.1);padding:10px 12px;border-radius:8px;width:100%;appearance:none">
        <option value="">-- Seçin --</option>
        ${countrySel}
      </select>
    </div>
    <div class="field-row" id="state-row">
      <div class="field-label">Şehir/Eyalet</div>
