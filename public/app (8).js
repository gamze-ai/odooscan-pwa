...
function renderNewContactForm(prefill) {
  ...
  html += makeInput('name', 'Ad Soyad', prefill.name || '', true);
  ...
  html += makeInput('website', 'Website', prefill.website || '');

  html += `<p class="section-label" style="margin-top:20px;margin-bottom:12px">📍 ADRES</p>`;
  html += makeInput('street', 'Adres', prefill.street || '');
  html += makeInput('city', 'İlçe', prefill.city || '');

  // Şehir (state) dropdown
  const states = opts.states || [];
  if (states.length > 0) {
    const stateSel = states.map(s => `<option value="${s.id}" ${prefill.state_id == s.id ? 'selected' : ''}>${esc(s.name)}</option>`).join('');
    html += `
      <div class="field-row">
        <div class="field-label">Şehir</div>
        <select class="field-input" id="nc-state_id"
          style="background:var(--surface2);color:var(--text);border:1px solid rgba(255,255,255,.1);padding:10px 12px;border-radius:8px;width:100%;appearance:none">
          <option value="">-- Seçin --</option>
          ${stateSel}
        </select>
    ...
    html += makeInput('state', 'Şehir', prefill.state || '');
  }

  html += makeInput('zip', 'Posta Kodu', prefill.zip || '');

  // Ülke dropdown
  const countries = opts.countries || [];
  if (countries.length > 0) {
    const countrySel = countries.map(c => `<option value="${c.id}" ${prefill.country_id == c.id ? 'selected' : ''}>${esc(c.name)}</option>`).join('');
    html += `
      <div class="field-row">
        <div class="field-label">Ülke</div>
        <select class="field-input" id="nc-country_id"
          style="background:var(--surface2);color:var(--text);border:1px solid rgba(255,255,255,.1);padding:10px 12px;border-radius:8px;width:100%;appearance:none">
          <option value="">-- Seçin --</option>
          ${countrySel}
        </select>
    ...
    html += makeInput('country', 'Ülke', prefill.country || '');
  }

  html += `<p class="section-label" style="margin-top:20px;margin-bottom:12px">⚙️ ODOO ALANLARI <span style="color:var(--danger)">*</span></p>`;
...
function submitNewContact() {
  // Kişi bilgilerini inputlardan oku
  const contactKeys = ['name','function','phone','mobile','email','website','street','city','state','zip','country'];
  const newData = {};
  contactKeys.forEach(k => {
    const el = document.getElementById('nc-' + k);
    if (el) newData[k] = el.value.trim();
  });
  // Dropdown ID alanları
  const stateEl = document.getElementById('nc-state_id');
  if (stateEl?.value) newData.state_id = parseInt(stateEl.value);
  const countryEl = document.getElementById('nc-country_id');
  if (countryEl?.value) newData.country_id = parseInt(countryEl.value);
  ...
  // Odoo zorunlu alanlarını oku
  const requiredLabels = {
    user_id: 'Satış Temsilcisi',
    property_payment_term_id: 'Müşteri Ödeme Koşulu',
    property_supplier_payment_term_id: 'Tedarikçi Ödeme Koşulu',
    sale_currency_rate_type_id: 'Satış Kur Türü',
  };
  const required = Object.keys(requiredLabels);
  for (const k of required) {
    ...
    if (!state.manualFields[k]) {
      toast(`❌ "${requiredLabels[k]}" alanı zorunludur`, 'error');
      el && el.focus();
      return;
    }
  }
