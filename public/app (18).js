...
async function doLogin() {
  ...
  try {
    ...
    if (data.success) {
      const user = { name: data.name, email: data.email, uid: data.uid, api_key: data.api_key };
      state.currentUser = user;
...
function renderHistory(searchQuery, filterUser, filterCompany, filterFuar, filterAksiyon) {
  ...
  const filtered = hist.map((item, idx) => ({ item, idx })).filter(({ item }) => {
    ...
    const searchable = [
      ...
      ff.gorusulen_3,
      ff.gorusme_yapan_1, ff.gorusme_yapan_2, ff.gorusme_yapan_3,
      ff.notlar, ff.aksiyon_plan, ff.oncelik,
      item.scannedBy, item.fileName,
      item.result?.visit_id ? 'Ziyaret #' + item.result.visit_id : '',
      item.result?.contact_id ? 'Kontak #' + item.result.contact_id : '',
    ].map(v => (v || '').toLowerCase());
    return searchable.some(s => s.includes(q));
  });

  const makeSelect = (id, placeholder, options, val, extraArgs) => {
    if (!options.length) return '';
    const opts = options.map(o => '<option value="' + esc(o) + '"' + (val === o ? ' selected' : '') + '>' + esc(o) + '</option>').join('');
    return '<select id="' + id + '" class="field-input" onchange="renderHistory(' +
      "document.getElementById('hist-search')?.value," +
      "document.getElementById('hist-filter-user')?.value," +
      "document.getElementById('hist-filter-company')?.value," +
      "document.getElementById('hist-filter-fuar')?.value," +
      "document.getElementById('hist-filter-aksiyon')?.value" +
      ')" style="background:var(--surface2);color:var(--text);appearance:none;font-size:12px">' +
      '<option value="">' + placeholder + '</option>' + opts + '</select>';
  };

  const activeFilters = [fu,fc,ff2,fa].filter(Boolean).length;
  let controlsHtml = '<div style="margin-bottom:16px;display:flex;flex-direction:column;gap:8px">' +
    '<input id="hist-search" class="field-input" placeholder="🔍 Ara... (isim, şirket, fuar, notlar, tarayan...)" value="' + esc(q) + '" ' +
    ...
    '<span style="font-size:11px;color:var(--muted)">' + filtered.length + ' sonuç · toplam ' + hist.length + ' kayıt</span>' +
    (q || activeFilters ? '<button onclick="renderHistory()" style="font-size:11px;color:var(--danger);background:none;border:none;cursor:pointer">✕ Filtreleri Temizle</button>' : '') +
    '</div></div>';

  if (!filtered.length) {
    body.innerHTML = controlsHtml + '<div class="empty-state"><div class="ei">🔍</div><p>Sonuç bulunamadı</p></div>';
    return;
  }
  ...
  filtered.forEach(({ item, idx }) => {
    ...
    if (sid && !seenSessions[sid]) {
      ...
      listHtml += '<div style="display:flex;align-items:center;gap:8px;margin:16px 0 8px">' +
    ...
    listHtml += '<div class="hist-item" onclick="showHistoryDetail(' + idx + ')" style="cursor:pointer' + (sid ? ';border-left:3px solid var(--warn);padding-left:13px' : '') + '">' +
  ...
  body.innerHTML = controlsHtml + listHtml;
}
  ...
  overlay.innerHTML = `
    <div style="max-width:600px;margin:0 auto;background:var(--surface);border-radius:var(--radius);padding:24px">
      ...
      ${item.fileBase64 ? `<div style="margin-bottom:20px">
        ...
        <img src="data:${item.fileMime||'image/jpeg'};base64,${item.fileBase64}" style="width:100%;border-radius:8px;max-height:300px;object-fit:contain;background:var(--surface2)"/>
      </div>` : ''}
...
if (_savedUser) {
  ...
  // home screen zaten aktif, refreshHome çağır
  refreshHome();
} else {
