...
let state = {
  ...
  matchStep: 'contact',
  odooOptions: {}, // Odoo'dan çekilen dropdown seçenekleri
};
...
function buildMatchQueue() {
  ...
  // Kartvizit kişisi (görüşülen alanlardan farklıysa)
  const cardName = cf.name || '';
  const alreadyInQueue = queue.some(q => q.name.toLowerCase() === cardName.toLowerCase());
  if (cardName && !alreadyInQueue) queue.push({
